...
async function updateClothe(req, res) {
  ...
  let foundClothe = await Clothe.get(clotheId);
  if (foundClothe) {
...
async function deleteClothe(req, res) {
  let clotheId = parseInt(req.params.id);
  let deleteClothe = await Clothe.delete(clotheId);
